---
id: src/core/coding-types.ts
type: module
file: src/core/coding-types.ts
area: core
---

# src/core/coding-types.ts

*Module* · area [[core]] · 72 lines

> Qualitative text-coding project model (lives in the main store, saved with the project).

## Tested by
- [[features.test.ts]] · import
- [[navigation-audit.test.tsx]] · import
- [[palette.test.tsx]] · import
- [[shell-fixes.test.ts]] · import
- [[scenarios.test.ts]] · import
- [[units.test.ts]] · import
- [[actions.test.ts]] · import
- [[ai-budget.test.ts]] · import
- [[analysis.test.ts]] · import
- [[codebook.test.ts]] · import
- [[dataset.test.ts]] · import
- [[importers.test.ts]] · import
- [[outputs.test.ts]] · import
- [[reliability.test.ts]] · import
- [[rules.test.ts]] · import
- [[segments.test.ts]] · import
- [[project.test.ts]] · import

## Imported by
- [[store.ts]] · type-only, value
- [[coding/actions.ts]] · type-only
- [[AnalyseView.tsx]] · type-only
- [[CodebookPanel.tsx]] · type-only
- [[AiDialogs.tsx]] · type-only
- [[ImportDialog.tsx]] · type-only
- [[coding/hooks.ts]] · type-only
- [[QuickCode.tsx]] · type-only
- [[Reader.tsx]] · type-only
- [[ResponsesView.tsx]] · type-only
- [[ui.tsx]] · type-only
- [[uiStore.ts]] · type-only
- [[install.ts]] · type-only
- [[fileActions.ts]] · value
- [[projectFile.ts]] · type-only, value
- [[coding.ts]] · type-only
- [[assistant/types.ts]] · type-only
- [[coding/ai.ts]] · type-only
- [[coding/analysis.ts]] · type-only
- [[codebookIO.ts]] · type-only
- [[docxExports.ts]] · type-only
- [[example.ts]] · type-only
- [[exports.ts]] · type-only
- [[outputs.ts]] · type-only
- [[coding/reliability.ts]] · type-only
- [[rules.ts]] · type-only
- [[segments.ts]] · type-only
- [[survey.ts]] · type-only
- [[toDataset.ts]] · type-only
- [[tree.ts]] · type-only
- [[features.test.ts]] · value
- [[navigation-audit.test.tsx]] · value
- [[palette.test.tsx]] · value
- [[shell-fixes.test.ts]] · value
- [[scenarios.test.ts]] · value
- [[units.test.ts]] · value
- [[actions.test.ts]] · value
- [[ai-budget.test.ts]] · type-only
- [[analysis.test.ts]] · type-only
- [[codebook.test.ts]] · type-only
- [[dataset.test.ts]] · type-only
- [[importers.test.ts]] · type-only
- [[outputs.test.ts]] · type-only
- [[reliability.test.ts]] · type-only
- [[rules.test.ts]] · type-only
- [[segments.test.ts]] · type-only
- [[project.test.ts]] · value

## Types
CodeDef (line 3) · TextDoc (line 20) · CodedSegment (line 34) · Memo (line 49) · CodingProject (line 60)

## Symbols

### emptyCodingProject
*function* · line 69 · exported
- Used in: [[store.ts]], [[fileActions.ts]], [[projectFile.ts]], [[features.test.ts]], [[navigation-audit.test.tsx]], [[palette.test.tsx]], [[shell-fixes.test.ts]], [[scenarios.test.ts]], [[units.test.ts]], [[actions.test.ts]], [[project.test.ts]]
