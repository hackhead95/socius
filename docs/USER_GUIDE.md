...
Settings are stored in this browser only, and keys only when you tick **Remember this key on this computer**. Keys are never saved in project files, exports, the error log or the connection report. **Forget key** removes a key (do this on a shared computer).
...
**While you wait.** Under an explanation or an assistant answer, a line says what the AI is doing and for how long: **Choosing model**, **Waiting for Google**, **Thinking**, **Writing**. Answers appear as they are written. If Google's free per-minute limit is reached, the line counts down ("Waiting 11 s for Google's free limit...") and the request continues by itself.

**If something goes wrong.** Messages say what happened in plain words: a key that was not accepted (paste it again, or create a new one), too many requests (free tiers have per-minute and per-day limits; Socius waits out a short per-minute limit by itself), a browser that cannot run the on-device model (use Chrome or Edge, or choose Gemini), or a reply in an unexpected format (try again with fewer items; small on-device models find long lists harder). Where an AI answer failed (an explanation, the assistant), a **Details** link shows a short technical report you can copy for support. See [AI does not connect](#ai-does-not-connect) in Troubleshooting.
...
- **Too many requests.** Socius learns each model's per-minute limit from Google's answer and spaces requests to fit; a short wait shows as a countdown. If the message still appears, wait a minute. The daily limit starts again at midnight Pacific time (morning in Europe, early afternoon in India). Flash-Lite has far more free requests than Flash (about 5 a minute on the free tier): leave Model on **Automatic: Flash-Lite**.
...
If you need help, click **Copy details** under the checklist and paste it into your feedback (**Help > Send feedback or report a problem**). The report lists the app version, your browser, each step, what Google or the service answered and how long each step took. It never contains your key.

A failed test is shown in the settings and on the AI chip ("not connected"); it is logged as a warning, not as an error of Socius, so the Help menu gets no dot for it.

### AI replies are slow

- Leave **Model** on **Automatic: Flash-Lite**. Flash-Lite starts answering in about a second; Flash thinks longer first and has only about 5 free requests a minute, so it often has to wait.
- The line under the answer says where the time goes: **Waiting for Google** (the network or Google is slow), **Thinking**, **Writing**, or a countdown for Google's free limit.
- The first AI request after opening Socius also asks Google which models your key may use; Socius remembers the list for a day, so later requests skip that step.
- **Help > Error log** lists an **Info** entry for each AI request with its timings (no text of your data or the answer), and **Copy details** includes them. Include them when you report slowness.
- The on-device model is slower than online models, especially on computers without a strong graphics chip.

### Browser storage is full (autosave paused)

Socius saves your work in the browser as you go. Browsers give each website a storage allowance, and on hackhead95.github.io all sites share one allowance. The on-device AI models take 1 to 2 GB of it each. When it is full, a banner says **Browser storage is full, so autosave is paused**:

- Click **Save project** to save your work to a file now.
- Click **Manage storage** to see what uses the space (downloaded AI models, projects and autosave, the error log) and delete downloaded models you do not need. Autosave starts again by itself as soon as a save works; the banner then goes away.
- **Help > About Socius** also shows the storage used. If the browser allows it, **Ask the browser to keep it** stops the browser from clearing Socius's data when the disk is nearly full.
- A model download that runs out of space is removed automatically, so a failed download does not keep the storage full.
