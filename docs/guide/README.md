...
node docs/guide/tools/capture-screenshots.mjs            # raw PNGs in /tmp/guide-agent/raw (--base, --out to change)
...
`tools/capture-screenshots.mjs` drives the built app with Playwright at 1440 x 900 (device scale factor 2, light theme) on the bundled sample survey. It follows the guide's own steps (the Home screen, Define variable properties, recode age, reverse-code `trust3`, build the trust scale, run each analysis, search with Ctrl+K, load the worked coding example, code a sample interview, add a second coder), draws the numbered orange callouts into the page before each picture, and crops to the relevant part. The browser uses the en-GB locale, so dates read "24 Sep 2026, 14:55". The Assistant button is docked at the right end of the view tabs and stays in the pictures. Run it with section names (`tour`, `home`, `open`, `variables`, `define`, `prepare`, `analyses`, `charts`, `report`, `saving`, `search`, `help`, `codingmenu`, `coding`, `interviews`, `ai`, `local`) to capture only some, but note that later sections rely on the variables created in `prepare`. `tools/process-images.py` scales the pictures to at most 1600 px wide and saves them as 256-colour PNGs (about 3.7 MB in total).

The `help`, `ai` and `local` sections each run in a fresh browser context (a first visit). `help` serves the build as `https://hackhead95.github.io/socius/` through a route (so addresses in the pictures are the real ones), stores a small dummy on-device model in Cache Storage, opens a text file renamed to `.sav` and makes one Gemini request fail, then photographs the Help menu with its blue dot, Help > Error log, the feedback dialog and the storage part of Help > About. `ai` walks through the AI chapter: the AI menu, Google Gemini set-up with an obviously fake `AQ.` key, a refused key (the failed Test connection checklist), a successful Test connection with the "AI is ready" panel, the AI chip, Explain with AI on the gender by `trust5` crosstab, the assistant panel (a t-test question and a trust-scale proposal) and the Text coding AI menu. `local` also serves the build as the live site, starts the stand-in Ollama from `scripts/diagnostics/fake-ollama.mjs` on port 11434 (skipped if the port is taken), grants Chrome's local network permission and photographs the Ollama check stopping at "Does it allow this website?" with the Windows steps. No request reaches Google: the script answers the Gemini Interactions API (`POST /v1beta/interactions`, see `e2e/gemini-mock.ts`) itself with Playwright routes (the model list, a plain reply for Test connection, a streamed explanation, and function calls for the assistant, whose tools run on the live data in the page). The AI texts are written by hand to match the sample survey's real results, so the captions in the guide call them example answers. Change them in `EXPLAIN_TEXT` and `assistantParts()` if the analyses change.
