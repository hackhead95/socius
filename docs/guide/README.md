...
`tools/capture-screenshots.mjs` drives the built app with Playwright at 1440 x 900 (device scale factor 2, light theme) on the bundled sample survey. It follows the guide's own steps (recode age, reverse-code `trust3`, build the trust scale, run each analysis, load the worked coding example, code a sample interview, add a second coder), draws the numbered orange callouts into the page before each picture, and crops to the relevant part. Run it with section names (`tour`, `open`, `variables`, `prepare`, `analyses`, `charts`, `report`, `saving`, `help`, `coding`, `interviews`, `ai`) to capture only some, but note that later sections rely on the variables created in `prepare`. `tools/process-images.py` scales the pictures to at most 1600 px wide and saves them as 256-colour PNGs (about 3.5 MB in total).
